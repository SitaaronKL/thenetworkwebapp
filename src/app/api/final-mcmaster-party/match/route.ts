...
import { FINAL_MCMASTER_PARTY_SLUG } from '@/lib/final-mcmaster-party-attendance';

const MATCH_REVEAL_ISO = '2026-03-28T00:00:00-04:00';
...
function isRevealTimeReached() {
  const revealAt = new Date(MATCH_REVEAL_ISO);
  const now = new Date();
...
export async function GET() {
  try {
    ...
    const revealAtIso = new Date(MATCH_REVEAL_ISO).toISOString();
    ...
    if (!activeMatch) {
      ...
      // Try forward match (user → candidates) via similarity scores
      ...
      if (forwardMatch?.match_user_id) {
        ...
        // Try reverse match (candidates → user)
      ...
      // No similarity data → random match from candidates
      if (!matchedUserId) {
        const randomIndex = Math.floor(Math.random() * candidateIds.length);
        matchedUserId = candidateIds[randomIndex];
      }
      ...
      const fallbackReason = sharedInterests.length > 0
        ? buildReasonFromSignals(sharedInterests, matchedExtras?.working_on ?? null)
        : 'You were matched at EVE — the final McMaster party. Say hi and see what happens.';
      ...
      if (insertMatchError) {
        // Race condition — another request may have created the match
